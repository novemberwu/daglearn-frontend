...
export default function TopicModulePage({ params }: PageProps) {
  ...
  // In User Story 1, this will come from the auth context.
  const userId = "user-1"; 

  useEffect(() => {
    async function loadMcqs() {
      try {
        const data = await apiService.getMcqsByTopic(topicId);
        setMcqs(data);
      } catch (error) {
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="space-y-8">
        ...
        {isLoading ? (
          ...
          mcqs.map((mcq) => (
            <McqCard key={mcq.id} mcq={mcq} userId={userId} onResult={handleResult} />
          ))
        ) : (
